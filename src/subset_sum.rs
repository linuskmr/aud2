...
pub fn subset_sum_full_bool_table(numbers: &[u64]) -> Vec<Vec<bool>> {
    ...
    for new_number in numbers {
        ...
        for (num_last_row, producible_last_row) in last_row.iter().enumerate() {
            ...
            match new_row.get_mut(num_last_row + (*new_number as usize)) {
                // New number producible
                Some(cell) => *cell = true,
                // Cell out ouf bounds, so we are not interested in it
                None => (),
            }
        }
        table.push(new_row);
...
mod test {
    ...
    #[test]
    fn test_subset_sum() {
        let actual_table = subset_sum_row_sum_set(&[7, 13, 17, 20, 29, 31, 31, 35, 57]);
        let expected_table = [
            HashSet::from([0]),
            HashSet::from([7, 0]),
